...
class MedistockDepartment(models.Model):
    department_id = models.AutoField(primary_key=True, db_column='dept_id')
    department_name = models.CharField(max_length=255, db_column='dept_name')
...
class MedistockLocation(models.Model):
    ...
    trail = models.CharField(max_length=120, blank=True, null=True)
    department = models.ForeignKey(MedistockDepartment, models.DO_NOTHING, db_column='dept_id', blank=True, null=True)
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(blank=True, null=True)
