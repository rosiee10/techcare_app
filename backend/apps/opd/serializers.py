...
from .models import EmergencyContact, PatientProfiling, OpdRoomLocation, OpdServiceSchedule
...
class ServiceScheduleSerializer(serializers.ModelSerializer):
    ...
    def get_is_open_today(self, obj):
        """Return active_today from database (auto-updated by view based on schedule)"""
        return obj.active_today
    ...
    def get_color_hex(self, obj):
        """Return color_theme from database, fallback to default"""
        if obj.color_theme:
            return obj.color_theme
        # Fallback color mapping for existing services without color_theme
        color_map = {
            'FAMED': '#2196F3',
            'PEDIA': '#4CAF50',
            'DENTAL': '#00BCD4',
        ...
        return color_map.get(obj.service, '#2196F3')


class RoomLocationSerializer(serializers.ModelSerializer):
    """Serializer for OPD Room Location"""
    id = serializers.IntegerField(source='room_location_id', read_only=True)
    code = serializers.CharField(max_length=10, required=False, allow_blank=True, allow_null=True)
    name = serializers.CharField(source='room_name', max_length=80, required=False, allow_blank=True, allow_null=True)
    floor = serializers.CharField(source='room_type', max_length=30, required=False, allow_blank=True, allow_null=True)
    service = serializers.CharField(max_length=30, required=False, allow_blank=True, allow_null=True)
    queuePrefix = serializers.CharField(source='q_prefix', max_length=2, required=False, allow_blank=True, allow_null=True)
    doctor = serializers.CharField(required=False, allow_blank=True, allow_null=True)
    status = serializers.CharField(max_length=10, required=False, allow_blank=True, allow_null=True)
    
    class Meta:
        model = OpdRoomLocation
        fields = ['id', 'code', 'name', 'floor', 'service', 'queuePrefix', 'doctor', 'status']
    
    def to_representation(self, instance):
        """Override to return doctor display name on read"""
        data = super().to_representation(instance)
        # Use the get_doctor logic for read operations
        data['doctor'] = self._get_doctor_display(instance)
        return data
    
    def create(self, validated_data):
        """Override create to handle doctor field mapping"""
        # Extract doctor field if present and map to doctor_id
        doctor = validated_data.pop('doctor', None)
        
        # Create the room instance
        room = OpdRoomLocation.objects.create(**validated_data)
        
        # If doctor was provided, update doctor_id
        if doctor:
            try:
                doctor_id = int(doctor) if isinstance(doctor, str) else doctor
    ...
    def _get_doctor_display(self, obj):
        """Get doctor display name from doctor_id"""
        import logging
        logger = logging.getLogger(__name__)
        
        logger.info(f"[DEBUG] _get_doctor_display called - doctor_display: {obj.doctor_display}, doctor_id: {obj.doctor_id}")
        
        if obj.doctor_display:
            logger.info(f"[DEBUG] Returning doctor_display: {obj.doctor_display}")
            return obj.doctor_display
        
        if obj.doctor_id:
            from django.db import connection
            try:
                doctor_id = int(obj.doctor_id) if isinstance(obj.doctor_id, str) else obj.doctor_id
                logger.info(f"[DEBUG] Looking up doctor_id: {doctor_id} (type: {type(doctor_id)})")
                ...
                with connection.cursor() as cursor:
                    query = """
                        ...
                        WHERE user_id = %s
                    """
                    logger.info(f"[DEBUG] Executing query: {query} with doctor_id: {doctor_id}")
                    ...
                    if result:
                        ...
                        logger.info(f"[DEBUG] Returning formatted doctor_display: {doctor_display}")
                        return doctor_display
                    else:
                        logger.warning(f"[DEBUG] User not found for doctor_id: {doctor_id}")
                        return str(obj.doctor_id)
            except Exception as e:
                logger.error(f"[DEBUG] Exception in _get_doctor_display: {str(e)}", exc_info=True)
                return str(obj.doctor_id)
        
        logger.info(f"[DEBUG] No doctor_id found, returning None")
        return None


class OpdServiceSerializer(serializers.ModelSerializer):
    """Serializer for OPD Services"""
    id = serializers.IntegerField(read_only=True)
    code = serializers.CharField(source='service', max_length=10)
    name = serializers.CharField(source='service_label', max_length=80, required=False, allow_blank=True, allow_null=True)
