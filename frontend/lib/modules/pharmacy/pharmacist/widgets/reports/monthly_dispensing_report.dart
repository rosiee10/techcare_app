...
/// Monthly Dispensing Report widget - exact physical form layout (same as quarterly but for 1 month)
class MonthlyDispensingReport extends StatelessWidget {
  final Map<String, dynamic> data;

  const MonthlyDispensingReport({super.key, required this.data});
  ...
  /// Build the PDF table matching the exact physical form layout (for 1 month)
  static pw.Widget buildPdf(List<String> months, List<Map<String, dynamic>> medicines, [String reportTitle = 'MONTHLY REPORT']) {
    if (months.isEmpty || medicines.isEmpty) {
      return pw.Text('No dispensing data available');
    }

    const double nameColWidth = 340;
    const double ipdColWidth = 65;
    const double opdColWidth = 65;
    const double totalColWidth = 65;
    ...
    // Build month header rows (only 1 month)
  ...
  /// Print the monthly dispensing report as PDF with the exact same design
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            buildPdf(months, medicines, reportTitle),
          ];
        },
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final reportTitle = data['report_title'] ?? 'MONTHLY REPORT';
    final screenWidth = MediaQuery.of(context).size.width;
    final isMobile = screenWidth < 600;

    if (medicines.isEmpty) {
      return const Center(child: Text('No dispensing data available for this month'));
    }

    final double nameColWidth = isMobile ? 150 : 380;
    final double ipdColWidth = isMobile ? 40 : 100;
    final double opdColWidth = isMobile ? 40 : 100;
    final double totalColWidth = isMobile ? 40 : 100;
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            // Right: Month headers (only 1 month)
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                // Month names row
          ...
          Builder(builder: (context) {
            ...
            return Row(
              ...
              children: [
                ...
                // Month data columns (only 1 month)
