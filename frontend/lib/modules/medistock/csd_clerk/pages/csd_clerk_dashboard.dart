...
import '../widgets/homepage/mobile_recent_activity_section.dart';
...
class CsdClerkDashboard extends StatefulWidget {
  const CsdClerkDashboard({super.key});

  @override
  State<CsdClerkDashboard> createState() => _CsdClerkDashboardState();
}

class _CsdClerkDashboardState extends State<CsdClerkDashboard> {
  int _selectedIndex = 0; // Start with Dashboard selected

  // Mobile bottom nav items for CSD Clerk - 6 items matching sidebar
  final List<NavItem> _mobileNavItems = const [
    NavItem(icon: Icons.dashboard_outlined, label: 'Home'),
    NavItem(icon: Icons.inventory_2_outlined, label: 'Inventory'),
    NavItem(icon: Icons.shopping_cart_outlined, label: 'Carts'),
    NavItem(icon: Icons.request_page_outlined, label: 'Requests'),
    NavItem(icon: Icons.restaurant_outlined, label: 'Kitchen'),
  ...
  /// Mobile dashboard content with full header
  Widget _buildMobileDashboardContent() {
    return Container(
      color: Colors.white,
      child: CustomScrollView(
        slivers: [
          // Full Header: AppBar + Search + Profile
          ...
          // Stats section
          ...MobileStatsSection().build(context),
          // Recent Activity section
          ...MobileRecentActivitySection().build(context),
  ...
  Widget _buildContent() {
    switch (_selectedIndex) {
      ...
      case 2:
        return const InventoryCartsPage();
      case 3:
        return const PurchaseRequestPage();
      case 4:
        return const KitchenPage();
      case 5:
        return const ForecastingPage();
      case 6:
